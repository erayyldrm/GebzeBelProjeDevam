...
const WorkshopCenterCard = ({ center }: { center: Kutuphane }) => {
...
export default function KutuphanePage() {
    const [kutuphaneler, setKutuphaneler] = useState<Kutuphane[]>([]);
    const [loading, setLoading] = useState<boolean>(true);
    const [error, setError] = useState<string | null>(null);

    useEffect(() => {
        const fetchKutuphaneler = async () => {
            try {
                setLoading(true);
                const response = await axios.get('http://localhost:8080/api/hizmetler/kategori/KUTUPHANE');
                ...
                if (Array.isArray(response.data)) {
                    ...
                    setKutuphaneler(formattedData);
                } else {
                    throw new Error('Geçersiz veri formatı');
                }
            } catch (err) {
                console.error("API Hatası:", err);
                setError('Kütüphane hizmetleri yüklenirken bir hata oluştu');
            } finally {
                setLoading(false);
            }
        };
                ...
                <div className="flex-1 px-4 sm:px-6 pt-4 pb-20 w-full">
                    ...
                    <section className="mb-10 sm:mb-20">
                        <div className="mx-auto">
                            ...
                            {kutuphaneler.length === 0 ? (
                                <div className="text-center text-gray-500 p-6 sm:p-10">
                                    Herhangi bir kütüphane bulunamadı.
                                </div>
                            ) : (
                                /* Kartlar */
                                <div className="flex flex-col items-center w-full">
                                    {kutuphaneler.map((center) => (
