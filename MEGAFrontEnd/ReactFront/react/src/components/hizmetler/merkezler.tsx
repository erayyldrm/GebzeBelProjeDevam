...
const WorkshopCenterCard = ({ center }: { center: Merkez }) => {
        ...
        >
            ...
            {showDetails && (
                <div className="bg-gray-100 p-5 border-t border-gray-300">
                    <p className="text-sm md:text-base text-gray-700">{center.detaylar || "Detay bilgisi bulunmamaktadır."}</p>
                </div>
            )}
        </motion.div>
    );
...
export default function MerkezlerPage() {
    const [merkezler, setMerkezler] = useState<Merkez[]>([]);
    const [loading, setLoading] = useState<boolean>(true);
    const [error, setError] = useState<string | null>(null);

    useEffect(() => {
        const fetchMerkezler = async () => {
            try {
                setLoading(true);
                const response = await axios.get('http://localhost:8080/api/hizmetler/kategori/MERKEZ');
                ...
                if (Array.isArray(response.data)) {
                    ...
                    setMerkezler(formattedData);
                } else {
                    throw new Error('Geçersiz veri formatı');
                }
            } catch (err) {
                console.error("API Hatası:", err);
                setError('Merkezler yüklenirken bir hata oluştu');
            } finally {
                setLoading(false);
            }
        };
                        ...
                        <div className="mx-auto">
                            ...
                            {merkezler.length === 0 ? (
                                <div className="text-center text-gray-500 p-6 sm:p-10">
                                    Herhangi bir merkez bulunamadı.
                                </div>
                            ) : (
                                /* Kartlar */
